...
Phases 0–10. **Phase 0 is this documentation phase.** Do not start application code until Phase 0 exits.

Prefer a vertical slice (contracts + one live proof + thin UI) over polish when iterating quickly.
...
**Exit:** Documentation review of the Phase 0 baseline report.
...
**Tasks:** T-SEC remaining; ad hoc invariant notes; optional `slither` if it installs quickly — skip if tooling is slow.
...
**Tasks:** Follow `DEPLOYMENT_PLAN.md`. **Do not deploy during the documentation baseline.**
**Dependencies (historical):** A funded `DEPLOYER_PRIVATE_KEY` was required to exit.
...
**Objective:** Rehearse the live demo path; record a backup video if useful.
**Tasks:** Execute `DEMO_SPECIFICATION.md`; Q&A from COMPETITIVE_POSITIONING.
**Exit:** DEMO-005 spoken; operator checklist complete.

---

## Phase 10 — Public release

**Objective:** Public repo, optional demo video, addresses, writeup.
**Tasks:** Keep GitHub public; do not leak keys.
**Exit:** Docs, evidence JSON, and demo materials aligned with live CC3 deploy.
