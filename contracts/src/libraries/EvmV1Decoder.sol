...
/// @dev Vendored from @gluwa/asc-contracts@0.2.1 (`contracts/common/EvmV1Decoder.sol`).
///      Encoding matches Proof Builder `txBytes` / official CC3 decoder. Do not "simplify"
///      this to a demo-tx-only parser. Walk original logs in BlacklistVerifier; do not use
///      getLogsByEventSignature for binding (it drops logIndex).
...
library EvmV1Decoder {
    ...
    function _decodeTypeSpecificChunkType2(bytes memory chunk) private pure returns (Type2Fields memory f) {
        ...
        (f.chainId, f.maxPriorityFeePerGas, f.maxFeePerGas, al, f.yParity, f.r, f.s) =
            abi.decode(chunk, (uint64, uint128, uint128, AccessListEntryBytes32[], uint8, bytes32, bytes32));
